...
public class charubfs3
{
    ...
    {
        new charubfs3(); // I like doing things this way so I don't have to use static objects
    }

    charubfs3() throws FileNotFoundException
    {
       ...
       for (int k = 0; k < map.length; k++)
        {
            System.out.println(map[k]+" "+V[k]);
        }
        //System.out.println();
        //System.out.println("------------------------------\n"+ (degsiz -assg));
        //System.out.println();
        System.err.println("All done - have a good day!");
    }
    ...
    void BFS() throws FileNotFoundException
    {
        ...
        //System.err.println(" degree list size : " +degsiz );
        ...
        while (list.size() > 0)
        {
            System.err.println(iter++ + " " + list.size());
            ...
            {
		System.err.println("Calling BFS with"+degree.get(sample));
		...
		System.err.println("Calling BFS with binary and "+degree.get(list.get(temp)));
...
//        
	...
	System.err.println("Calling BFS()");
	    ...
	    V[at]=true;
            count++;
	          ArrayList<Integer> list1 = new ArrayList(degree.keySet());
                    ...
                    if (res == -1)
                    {
                        //System.err.println("check building of degree");
                    } else
                    {
                        int s1 = 0, temp1;
                        if (list1.lastIndexOf(res) != list1.size() - 1)
                        {
                            //System.err.println("updating degree list after" + res + " at " + i + " size = " + degree.keySet().size());
                            for (int l : list1.subList(list1.indexOf(res) + 1, list1.size() - 1))
                            {
                                //if (l > res)
                                {
...
//                                }
                                    if (l - res < 0)
                                    {
                                        //System.err.println("FAILLLLLLLLLLL");
                                    }
                                    degree.put(l - res, temp1);
                                    s1 = l - res;
                                }
                        ...
                        degree.remove(res);
                        list = new ArrayList(degree.keySet());
                        //System.out.println("listing size=" + list.size());
                    }
                    num++;
